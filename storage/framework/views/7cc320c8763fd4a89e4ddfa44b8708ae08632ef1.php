<?php
    ...
    $dsdichvu = App\Http\Services\ContentService::getCmsDichvu($params)->get();
?>
<div class="wprm-wrapper d_mb" role="navigation">
    ...
    <div id="wprmenu_bar" class="wprmenu_bar bodyslide left widget-menu-left wpr-logo-left">
        ...
        <div class="wprm_logo">
            <a href="/">
            	<img data-lazyloaded="1" src="<?php echo e($web_information->image->logo_header ?? ''); ?>"
                width="302" height="62" data-src="<?php echo e($web_information->image->logo_header ?? ''); ?>" alt="<?php echo e($web_information->information->site_name ?? ''); ?>">
            </a>
        </div>
            ...
            <?php
              ...
              foreach ($menu as $key => $item) {
                  ...
                  if ($item->parent_id == $main_menu->id) {
                      ...
                      //cấp 2
                      if ($item->sub > 0) {
                         $content .= '<li id="menu-item-{{ $item->id }}" class="menu-item menu-item-has-children"><a target="'.$target.'" href="' . $url . '" aria-current="page">' . $title . '</a>';
                          $content .= '<ul class="sub-menu" style="text-align: start">';
                          foreach ($menu as $item_sub) {
                              ...
                              if ($item_sub->parent_id == $item->id) {
                                  ...
                                  $content .= '<li id="menu-item-{{ $item_sub->id }}" class="menu-item"><a target="'.$target.'" href="' . $url . '">'.$title.'</a>';
                                  $content .= '</li>';
                              }
                          }
                          $content .= '</ul>';
                      } else {
                        $content .= '<li id="menu-item-{{ $item->id }}" class="menu-item"><a target="'.$target.'" href="' . $url . '" aria-current="page" style="text-decoration: none;">' . $title . '</a>';
                      } 
                      $content .= '</li>';
                  }
              }
            ...
            <div class="entry-loop__row">
                ...
                <div class="form_register">
                    ...
                    <form method="POST" action="<?php echo e(route('frontend.contact.store')); ?>">
                        <?php echo csrf_field(); ?>
                        <input type="text" name="name" id="name" placeholder="Họ và tên (*)" aria-label="Họ và tên" required>
    ...
    <?php /**PATH /home/ntdconyj0yk6/phongkham.nvoting.com/resources/views/frontend/element/menu.blade.php ENDPATH**/ ?>